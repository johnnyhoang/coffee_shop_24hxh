  ...
  return (
    <AriaListBoxItem {...props} textValue={textValue} className={itemStyles}>
      {composeRenderProps(props.children, (children) => (
        <>
          {children}
          <div className="absolute left-100 right-4 bottom-0 h-px bg-white/20 forced-colors:bg-[HighlightText] hidden [.group[data-selected]:has(+[data-selected])_&]:block" />
        </>
      ))}
    </AriaListBoxItem>
  );
...
export const dropdownItemStyles = tv({
  base: 'group flex items-center gap-0 cursor-default py-2 pl-3',
  variants: {
    isDisabled: {
  ...
  return (
    ...
    >
      {composeRenderProps(props.children, (children, { isSelected }) => (
        <>
          <span className="flex-1 flex items-center gap-1 truncate font-normal group-selected:font-semibold">
            {children}
          </span>
